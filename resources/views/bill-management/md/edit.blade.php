<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800 leading-tight">
            {{ __('Medical Director Clinical Authorization') }}
        </h2>
    </x-slot>

    <div class="max-w-7xl mx-auto p-6 pb-24" x-data="mdEditApp({
        pa_code: '{{ $log->pa_code }}',
        ...
        <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-lg z-50">
            <div class="max-w-7xl mx-auto flex justify-between items-center">
                <div class="flex space-x-12">
                    ...
                    <div>
                        <p class="text-[10px] text-red-500 uppercase font-black">Authorized Due</p>
                        <p class="text-2xl font-black text-red-700">₦<span x-text="formatMoney(totalDue)"></span></p>
                    </div>
                </div>
                <button @click="submitUpdate" :disabled="isSaving"
                    class="bg-red-600 hover:bg-red-700 text-white px-12 py-3 rounded-xl font-bold shadow-lg transition-all active:scale-95 disabled:opacity-50 uppercase tracking-widest">
                    <span x-text="isSaving ? 'Authorizing...' : 'Final Clinical Approval'"></span>
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        function mdEditApp(data) {
            return {
                ...
                submitUpdate() {
                    if(!confirm('Provide final clinical authorization?')) return;
                    this.isSaving = true;
                    let url = "{{ route('bill-management.md.update', 'ID_HERE') }}".replace('ID_HERE', data.encoded_pa);
                    ...
                    .then(res => {
                        window.location.href = "{{ route('bill-management.md.index') }}?message=Authorized Successfully";
                    })
                    .catch(err => {
                        this.isSaving = false;
