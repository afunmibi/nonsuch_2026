    ...
    <script>
        function udEditApp(data) {
            return {
                ...
                submitUpdate() {
                    if(!confirm('Authorize these changes?')) return;
                    this.isSaving = true;
                    let url = "{{ route('bill-management.ud.update', 'ID_HERE') }}".replace('ID_HERE', data.encoded_pa);
                    ...
                    .then(res => {
                        window.location.href = "{{ route('bill-management.ud.index') }}?message=Authorized Successfully";
                    })
                    .catch(err => {
                        this.isSaving = false;
